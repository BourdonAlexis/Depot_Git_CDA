...
abstract class AbstractController
{
    ...
    //validation picture
    public function validPicture(array $files)
    {
        $imgError = [];
        $aMimeTypes = array("image/gif", "image/jpeg", "image/pjpeg", "image/png", "image/x-png", "image/tiff", "image/jpg");
        // si le fichier upload
        $finfo = finfo_open(FILEINFO_MIME_TYPE);
        $mimetype = finfo_file($finfo, $files["picture"]["tmp_name"]);
        finfo_close($finfo);
        if (!in_array($mimetype, $aMimeTypes)) {
            // Message error
            $imgError['picture'] = ' *Format d\'image incorrecte (format valide : gif, jpeg, png, pjeg, x-png, tiff) ';

        } else {
            move_uploaded_file($files['picture']['tmp_name'], ROOT . '/assets/IMG/' . $files['picture']['name']);
        }
        return $imgError;
    }
